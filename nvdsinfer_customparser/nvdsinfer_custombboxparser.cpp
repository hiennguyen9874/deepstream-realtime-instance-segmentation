...
extern "C" bool NvDsInferParseCustomEfficientNMSTLTMask(
    std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
    NvDsInferNetworkInfo const &networkInfo,
    NvDsInferParseDetectionParams const &detectionParams,
    std::vector<NvDsInferInstanceMaskInfo> &objectList);
    ...
    std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
    ...
    */
    ...
    for (int i = 0; i < p_keep_count[0] && objectList.size() <= keep_top_k; i++) {
        if (p_classes[i] >= detectionParams.numClassesConfigured)
            continue;
...
CHECK_CUSTOM_INSTANCE_MASK_PARSE_FUNC_PROTOTYPE(NvDsInferParseCustomBatchedNMSTLTMask);
CHECK_CUSTOM_INSTANCE_MASK_PARSE_FUNC_PROTOTYPE(NvDsInferParseCustomEfficientNMSTLTMask);
